package com.kotlin.samples.flow
...
class FlowEntrance {
     ...
     * 流中可以使用挂起函数
     */
    private fun createFlowByflow(): Flow<Int> {
        return flow {
            delay(1000)
            emit(1)
            delay(1000)
     ...
     * 2.流创建:flowOf()
     */
    private fun createFlowByflowOf(): Flow<Int> {
     ...
     * 3.流创建:asFlow()
     */
    private fun createFlowByAsFlow(): Flow<Int> {
...
//                        }
    ...
    private suspend fun mapDelay() {
        delay(1000)
    }

    /**
     * 7.转换流操作符,可以再次发射任意值，任意次
     */
    private fun transformFlow() {
        val flow = flowOf("a", "b")
        val newFlow = flow.transform { req ->
            delay(100)
            emit(req)
            transformDelay()
            emit(req.uppercase())
    ...
    private suspend fun transformDelay() {
        delay(1000)
    }
     ...
     * 12.流的上下文
     */
    private fun contextWithFlow() {
        val flow = flow {
            withContext(Dispatchers.Default) {
                log("流发射:${Thread.currentThread().name}")
                emit(1)
            }
        }
        runBlocking {
    ...
    /**
     * 13.flowOn操作符，用于更改流发射的上下文,当必须更改[CoroutineDispatcher]时，flowOn采用了buffer缓冲机制
     */
    private fun flowOn() {
        val flow = flow {
            emit(1)
        ...
        runBlocking {
            launch {
                flow.collect {
                    log("流收集:$it ${Thread.currentThread().name}")
                }
            }
        }
    }
     ...
     * 14.buffer操作符
     */
    private fun bufferFlow() = runBlocking {
    ...
    private fun combineFlow() {
        val numFlow = (1..3).asFlow().onEach { delay(300) }
        val strFlow = flowOf("One", "Two", "Three").onEach { delay(400) }
        runBlocking {
            val startTime = System.currentTimeMillis()
            numFlow.combine(strFlow) { a, b -> "$a:$b" }
                .collect {
                    log("流收集: $it,${System.currentTimeMillis() - startTime}")
                }
        }
    }
    ...
    private fun flatMapConcatFlow() {
        ...
        val startTime = System.currentTimeMillis()
        runBlocking {
            numFlow.onEach { delay(100) }
                .flatMapLatest {
                    requestFlow(it)
                }.collect {
                    log("$it at ${System.currentTimeMillis() - startTime}")
                }
        }
//        val numFlow = flow {
...
//        }
    ...
    private fun cancelableFlow() {
        val flow = (1..3).asFlow().cancellable()
        val coroutineExceptionHandler = CoroutineExceptionHandler { coroutineContext, throwable ->
            log("捕获到的异常coroutineExceptionHandler:$throwable")
    ...
    2
    3
    这是返回一个流的 simple 函数没有标记 suspend 修饰符的主要原因。 The simple() call itself returns quickly and does not wait for anything. The flow starts afresh every time it is collected and that is why we see "Flow started" every time we call collect again.
    ...
    流构建器
    先前示例中的 flow { ... } 构建器是最基础的一个。还有其他构建器让流可以这样声明：
    ...
    过渡流操作符
    可以使用操作符转换流，就像转换集合与序列一样。 过渡操作符应用于上游流，并返回下游流。
